...
import {
  ...
  ArrowUpCircle, MessageSquare, ListChecks, CircleDot, ShieldCheck,
...
function StaffDirectory({ staff }: { staff: any[] }) {
  if (staff.length === 0) {
    return <Card><CardContent className="py-8 text-center text-gray-500">No staff members found</CardContent></Card>;
  }
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {staff.map((s: any) => {
            ...
            <CardContent className="py-4">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center">
                  <RIcon className="h-5 w-5 text-green-600" />
                </div>
                <div>
                  <p className="font-medium">{s.firstName} {s.lastName}</p>
                  <p className="text-xs text-gray-500">{ROLE_LABELS[s.role]}</p>
