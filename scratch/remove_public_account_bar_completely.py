...
export default function CargoPage() {
  ...
  const [customerAccount, setCustomerAccount] = useState({
    ...
    email: "",
    phone: "",
    accountType: "Enterprise Shipper Account",
      ...
      <main className="pb-20">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 mt-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
            ...
            {/* =================================================================== */}
            {/* TAB 3: B2B CUSTOMER PORTAL (CLEAN PUBLIC B2B HUB)                   */}
            {/* =================================================================== */}
            <TabsContent value="b2b-portal" className="space-y-8">
              
              {/* CLEAN PUBLIC B2B PORTAL HEADER BANNER */}
              <div className="bg-gradient-to-r from-slate-900 via-emerald-950 to-slate-900 border border-emerald-500/20 p-6 rounded-3xl text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      <h3 className="font-black text-xl text-white">TOTAG B2B Shipper Portal & Cargo Booking Vault</h3>
                      <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px]">
                        Enterprise Customer Portal
                      </Badge>
                    </div>
                    <p className="text-xs text-slate-300 mt-1 max-w-2xl">
                      Book new shipments, generate official printable Air Waybills (AWBs), and monitor demurrage exception alerts.
                    </p>
                  </div>
                </div>

                {customerAccount.isLoggedIn && (
                  <div className="flex items-center space-x-2 bg-emerald-500/10 border border-emerald-500/30 p-2.5 rounded-2xl">
                    <Building2 className="w-4 h-4 text-emerald-400" />
                    <div className="text-xs">
                      <span className="font-bold text-white block">{customerAccount.companyName}</span>
                      <span className="text-[10px] text-emerald-400 font-mono">TIN: {customerAccount.tinNumber}</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* MODAL 3: INTERACTIVE PASSWORD CHANGE MODAL (ACCESSED VIA ONBOARDING EMAIL OR LOGIN) */}
        <AnimatePresence>
          {isPasswordModalOpen && (
            <motion.div 
              initial={{ opacity: 0 }}
...
print("Removed pre-populated profile header bar completely from public page!")
